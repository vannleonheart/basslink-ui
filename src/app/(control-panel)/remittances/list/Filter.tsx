...
import { RemittanceStatuses, TransferTypes } from '@/data/static-data';
		...
		<div className="w-full flex flex-col gap-20 border px-12 py-12 rounded-lg mb-8 bg-gray-50">
			<div className="w-full grid grid-cols-1 gap-8 md:grid-cols-3 md:gap-16">
				...
				<TextField
					...
					fullWidth
				>
					{Object.keys(RemittanceStatuses).map((key) => (
						<MenuItem
							key={'status-' + key}
							value={key}
						>
							{RemittanceStatuses[key as keyof typeof RemittanceStatuses]}
						</MenuItem>
					))}
				</TextField>
				<TextField

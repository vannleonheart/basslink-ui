...
export default function DealListAdmin({ side, accessToken }: Required<AuthComponentProps>) {
		...
		<div className="bg-white rounded p-20 shadow-2">
			...
			<div className="flex flex-col">
				...
				<div className="w-full flex items-center justify-between text-sm mb-12 px-6">
					...
					<span className="w-2/12 font-bold">Recipient/Bank/Country</span>
...
function DealListItem({ deal, tab }: { deal: Deal; tab: string }) {
	...
	return (
		...
		>
			<div className="w-full py-28 flex items-start justify-between gap-4">
				...
				<div className="w-2/12 flex flex-col space-y-6">
					<span>{deal.receiver_name}</span>
					<span>{deal.receiver_bank_name}</span>
				...
				<div className="w-2/12 flex flex-col">{deal?.agent?.name}</div>
				<div className="w-1/12 flex flex-col">
					<StatusLabel
						status={deal.status}
						rounded

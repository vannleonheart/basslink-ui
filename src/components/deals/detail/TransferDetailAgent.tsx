...
export default function TransferDetailAgent({ deal }: { deal: Deal }) {
	...
	const clientCountryName = CountryCodeList.find((country) => country.code === deal?.client?.country)?.name;
	...
	return (
		<div className="flex flex-col gap-20">
			<div className="bg-gray-100 rounded-lg shadow-3 py-12 px-16">
				...
				<div className="w-full flex flex-row items-center justify-between bg-gray-200 px-20 py-16 rounded-xl">
					<div className="w-1/2 flex flex-col gap-6">
						<div className="font-bold text-lg">{deal?.client?.name}</div>
						<div>{clientCountryName}</div>
					</div>
					{deal?.agent_company && (
			...
			<Accordion className="bg-gray-300 rounded-t-lg">
